#include "Monster.h"
// #include <cstdlib> // 드랍 안 쓰면 굳이 필요 없음

// 멤버 초기화 리스트 / 이동 -> 
Monster::Monster(std::string n, int h, int a)
    : name(std::move(n)), health(h), attack(a)
{
}
...
// 노랭이 생성
Yellow::Yellow(int lvl)
    : Monster("야생의 노란 대머리", 40 + lvl * 5, 5 + lvl * 2) {
}

// 하양이 생성
White::White(int lvl)
    : Monster("신생아 머리", 60 + lvl * 6, 7 + lvl * 2) {
}

// 초록이 생성
Green::Green(int lvl)
    : Monster("쑥 대머리", 80 + lvl * 8, 9 + lvl * 3) {
}

// 파랭이 생성
Blue::Blue(int lvl)
    : Monster("착한 대머리", 80 + lvl * 8, 9 + lvl * 3) {
}

// 빨갱이 생성
Pink::Pink(int lvl)
    : Monster("나쁜 대머리", 80 + lvl * 8, 9 + lvl * 3) {
}

// 깜댕이 생성
Black::Black(int lvl)
    : Monster("진형형님", 120 + lvl * 10, 12 + lvl * 4) {
}
